...
class ChompEnv(gym.Env):
    def __init__(
        self, GRID_SIZE: Tuple[int, int] = (5, 10), opponent_mode: bool = True
    ) -> None:
        super().__init__()
        ...
        # critical moves
        self.poison_right_move: int = (
            self.POISON_RIGHT_NEIGHBOR_IDX[0] * self.NUM_COLS
        ) + self.POISON_RIGHT_NEIGHBOR_IDX[1]
        self.poison_up_move: int = (
            self.POISON_UP_NEIGHBOR_IDX[0] * self.NUM_COLS
        ) + self.POISON_UP_NEIGHBOR_IDX[1]
        self.poison_diag_move: int = (
            self.POISON_DIAG_NEIGHBOR_IDX[0] * self.NUM_COLS
        ) + self.POISON_DIAG_NEIGHBOR_IDX[1]
        ...
        self.observation_space: spaces.Box = spaces.Box(
            low=0, high=1, shape=self.GRID_SIZE, dtype=np.int8
        )
    ...
    def get_valid_actions(self) -> List[int]:
        ...
        return [
            ((row * self.NUM_COLS) + column) for row, column in edible_indices.tolist()
        ]
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        ...
        # critical states
        poison_diag_empty: bool = bool(
            state[self.POISON_DIAG_NEIGHBOR_IDX[0], self.POISON_DIAG_NEIGHBOR_IDX[1]]
            == 0
        )
        poison_up_empty: bool = bool(
            state[self.POISON_UP_NEIGHBOR_IDX[0], self.POISON_UP_NEIGHBOR_IDX[1]] == 0
        )
        poison_right_empty: bool = bool(
            state[self.POISON_RIGHT_NEIGHBOR_IDX[0], self.POISON_RIGHT_NEIGHBOR_IDX[1]]
            == 0
        )

        if (
            action not in valid_actions
        ):  # this is not foolproof because i don't think qlearning works like this
            reward = -1000.0
        ...
        if self.opponent_mode:
            opponent_valid_actions: List[int] = (
            ...
            if len(opponent_valid_actions) > 1:  # pick a random non poison move
                opponent_action: int = random.choice(
                    [a for a in opponent_valid_actions if a != self.poison]
                )
            ...
            reward += (
                150.0 if opponent_poison else 0.0
            )  # reward for the agent if opponent loses
    ...
    def render(self) -> str:
        ...
        board: str = ""
        header: str = "     " + "  ".join(
            f"[#D7A7E8]{chr(ord('A') + j)}[/#D7A7E8] " for j in range(self.NUM_COLS)
        )
        board += header + "\n"
