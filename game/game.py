...
def human_action(env: ChompEnv) -> Tuple[int, str]:
    ...
    if len(move) < 2 or not move[0].isalpha() or not move[1:].isdigit(): # filtering out invalid move formats
    ...
    if letter - 97 >= env.NUM_COLS or number - 1 >= env.NUM_ROWS: # filtering out moves that are out of bounds
    ...
    if action_index not in valid_actions: # filtering out moves that are no longer valid
        env.done = True
        ...
        while True:
            ...
            if env.done:
                if bot_action != env.poison: 
                    console.print("[bold #A0E8E3]the bot played an invalid move!") # handling invalid bot moves
                    sys.exit(1)

                console.print("[bold #A0E8E3]game over! the bot loses.[/bold #A0E8E3]") # handling bot loss
                sys.exit(0)
